...
local function createDangerZonesBoard()
    ...
    for i = 1, 10 do
        if memory.readbyte(ENEMY[i].ENEMY_TYPE) > 0 and memory.readbyte(ENEMY[i].ENEMY_TYPE) < 9 then
            ...
            danger_board = leftEnemyDangerZone(danger_board, enemy_spot, enemy_type, 2)    -- marking on map the danger zones, left neighbors
            danger_board = rightEnemyDangerZone(danger_board, enemy_spot, enemy_type, 2)   -- marking on map the danger zones, right neighbors
            danger_board = upEnemyDangerZone(danger_board, enemy_spot, enemy_type, 2)      -- marking on map the danger zones, up neighbors
            danger_board = downEnemyDangerZone(danger_board, enemy_spot, enemy_type, 2)    -- marking on map the danger zones, down neighbors
        end
    end

    if memory.readbyte(BOMB_ANIMATION_ADDRESS_1) ~= 0 or memory.readbyte(BOMB_ANIMATION_ADDRESS_2) ~= 0 or BOMB_FRAME_COUNT > 125 then
        for x = 1, bomb_power do
            for y = 1, bomb_power do
                if bomb_location[1]+x < BOARD_HEIGHT and danger_board[bomb_location[1]+x][bomb_location[2]] and (danger_board[bomb_location[1]+x][bomb_location[2]].TILE_INFO == 0 or danger_board[bomb_location[1]+x][bomb_location[2]].TILE_INFO == 9) then
...
-- function to calculate the manhattan distance of two positions
...
-- insert possibly paths to our queue
local function exploringAstar(queue, current_board, current_path, coord_x, coord_y, goal, g_cost, current_cost)
    if coord_x and coord_y then
        local new_path = createNewPath(current_path)
        local node = current_board[coord_x][coord_y]
        table.insert(new_path, node.COORDINATES)

        -- the g_cost acumulated
        local new_cost = current_cost + 1  -- every step increments our cost

        -- first time we check this path or is a cheaper path
        if g_cost[node] == nil or new_cost < g_cost[node] then
            g_cost[node] = new_cost

            -- calling our heuristic
            local h_cost = manhattanDistance(coord_x, coord_y, goal[1], goal[2])

            -- getting the f cost
            local f_cost = new_cost + h_cost

            -- inserting on the queue with the f cost
            table.insert(queue, {node = node, path = new_path, priority = f_cost})

            -- sorting the queue based on the f cost
...
local function searchForBonus()
    ...
    local goal = {BONUS_COORD[1], BONUS_COORD[2]} -- setting our objective (the bonus location)
    local queue = {{node = start_node, path = {start_node.COORDINATES}, priority = 0}}
    local visited = {}  -- table with all the visited nodes already
    local g_cost = {}   -- acumulated cost of every node
    g_cost[start_node] = 0

    while #queue > 0 do
        -- remove the lowest f cost from the queue
        local current = table.remove(queue, 1)
        local current_node = current.node
        local current_path = current.path
        local current_cost = g_cost[current_node]

        -- verify if we already visited this node
        if not visited[current_node] then
            visited[current_node] = true

            -- verify if the current node is our goal (any bonus in the map)
            if current_node.COORDINATES[1] == goal[1] and current_node.COORDINATES[2] == goal[2] then -- searching for the bonus location
                ...
                return true     -- return true if we find the bonus
            end
            ...
            exploringAstar(queue, BOARD_MAP, current_path, coord_x, coord_y, goal, g_cost, current_cost)
            ...
            -- verify down neighbor
            coord_x, coord_y = checkNeighborToBonus(current_node.DOWN_NEIGHBOR, visited)
            exploringAstar(queue, BOARD_MAP, current_path, coord_x, coord_y, goal, g_cost, current_cost)
        end
    end
    return false    -- return false if the bonus is no longer available 
end
...
local function searchForExit()
    -- starts the queue with the position of our player (a node) and his coordinates
    local start_node = BOARD_MAP[memory.readbyte(PLAYER_POSITION_X_ADDRESS)+1][memory.readbyte(PLAYER_POSITION_Y_ADDRESS)+1]
    local goal = {EXIT_DOOR_COORD[1], EXIT_DOOR_COORD[2]} -- setting our objective (the exit door location)
    local queue = {{node = start_node, path = {start_node.COORDINATES}, priority = 0}}
    local visited = {}  -- table with all the visited nodes already
    local g_cost = {}   -- acumulated cost of every node
    g_cost[start_node] = 0
...
local function searchForEnemy()
    local goal = {} -- initializing our goal as empty

    -- our goal will be the nearest enemy
    local player_coord = {memory.readbyte(PLAYER_POSITION_X_ADDRESS)+1, memory.readbyte(PLAYER_POSITION_Y_ADDRESS)+1}  -- our current coordinates
    local distance_goal = math.huge
    for i = 1, 10 do
        if memory.readbyte(ENEMY[i].ENEMY_TYPE) > 0 and memory.readbyte(ENEMY[i].ENEMY_TYPE) < 9 then
            local enemy_coord = {memory.readbyte(ENEMY[i].POSITION_X_ADDRESS)+1, memory.readbyte(ENEMY[i].POSITION_Y_ADDRESS)+1}  -- enemy coordinates
            local distance = manhattanDistance(player_coord[1], player_coord[2], enemy_coord[1], enemy_coord[2])
            if distance < distance_goal then
                distance_goal = distance
                goal = {enemy_coord[1], enemy_coord[2]}    -- nearest enemy
            end
        end
    end
    ...
    while #queue > 0 do
        ...
        if not visited[current_node] then
            ...
            if current_node.COORDINATES[1] == goal[1] and current_node.COORDINATES[2] == goal[2] then
                table.remove(current_path, 1)
                if #current_path > 0 then
                    ENEMY_COORD_BUFFER = {goal[1], goal[2]}
                end
                if #current_path > 0 and current_path[1][1] == goal[1] and current_path[1][2] == goal[2] then
                    if BOARD_MAP[memory.readbyte(PLAYER_POSITION_X_ADDRESS)+1][memory.readbyte(PLAYER_POSITION_Y_ADDRESS)+1].TILE_INFO ~= 9 then
                        putBomb()
                        BOMB_FRAME_COUNT = 0
                        BOMB_COUNT = BOMB_COUNT + 1
